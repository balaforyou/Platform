...
11. **Close the loop back to the reviewer conversation on everything this repo's own record shows Resolved, and on every direct hand-off the reviewer gives you — "merged" is not the same as "reported."** The reviewer conversation (Chief) maintains its own memory file outside this repo (a Claude Project doc, not git-tracked, so it can't be checked from here) that depends entirely on being told when something lands. This has failed for real: F-302 and F-297 were handed directly from the reviewer conversation to this repo with no completion report ever looping back, and a real security-hardening batch (F-290–F-296, F-298) plus two guest-pwa copy fixes (F-284, F-286) — real PRs #92–#98 and #82 — shipped with zero register/batch-log trace for 5+ days, only caught by a dedicated reconciliation sweep (25 Sep 2026). Every close-out report, whether routed through the Technical Lead thread or straight to Chief, states explicitly what landed (finding IDs, PR/commit, register status) so the reviewer's own tracking survives even though it lives outside this repo.
12. **Archive the final, approved version of a plan/handover into `docs/plans/chief-archive/` in the same PR that resolves the finding(s) it authorized.** The reviewer's plan/handover docs live outside this repo by default (a Claude Project, not git) — this makes the durable copy live here instead, alongside the register/batch-log entries the same PR already carries, so the reasoning behind a piece of code survives as long as the code does. Name the file after the finding ID(s), e.g. `docs/plans/chief-archive/f211-f237-f238-total-fix.md`. **The reviewer supplies the source content directly in the handover** (pasted in) — a repo-side agent has no other way to read a Project doc.
...
- **Google sign-in is real, not a mock — this changed with F-228 Step 1 and this note was stale.** `/auth/google/verify` performs real Google ID token verification (JWKS-based, no mock fallback in any environment); a sandboxed/local session cannot fabricate a valid token and so cannot drive guest-member-pwa's Gmail-only sign-in end-to-end (confirmed during F-263/F-266).
