...
> - **Google Signup gating**: During Google OAuth signup, a new account always routes through phone verification and lands as `GUEST` by default (same as OTP registration), with promotion to `MEMBER` or `STAFF` handled later by the secure, internal `PATCH /users/:id/type` endpoint.
