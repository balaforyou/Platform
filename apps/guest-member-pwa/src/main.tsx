      ...
      <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-gray-950/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <span className="font-bold text-lg tracking-wider uppercase font-outfit">
              {tenant?.appName}
            </span>
          </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-tr from-gray-900 to-gray-950 p-8 border border-white/5 shadow-2xl">
          <div className="absolute right-0 bottom-0 translate-x-1/4 translate-y-1/4 opacity-10">
            <Activity className="h-96 w-96 text-brand-primary" />
          </div>
          
          <div className="relative max-w-lg space-y-4">
            <div className="inline-flex items-center space-x-1 bg-brand-primary/10 border border-brand-primary/20 text-brand-primary px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider font-outfit">
              <MapPin className="h-3.5 w-3.5" />
              <span>{tenant?.name}</span>
            </div>
            <h2 className="text-4xl font-extrabold tracking-tight font-outfit md:text-5xl">
              Welcome back to <span className="text-brand-primary">{tenant?.appName}</span>
            </h2>
            <p className="text-gray-400 text-sm md:text-base leading-relaxed">
              Coimbatore's premium court booking platform. Find slots, book courts, and manage your matches.
            </p>
            <div className="pt-2 flex flex-wrap gap-4">
              <button className="py-3 px-6 rounded-2xl bg-brand-primary hover:opacity-95 text-white font-semibold flex items-center space-x-2 transition-all shadow-lg shadow-brand-primary/20">
                <span>Book Court Now</span>
                <ArrowRight className="h-4 w-4" />
              </button>
              <button className="py-3 px-6 rounded-2xl bg-white/5 hover:bg-white/10 text-gray-200 border border-white/10 font-semibold transition-all">
                View My Bookings
              </button>
            </div>
          </div>
          ...
          <div className="bg-white/5 p-6 rounded-2xl border border-white/5 space-y-4">
            ...
            <div className="text-xs text-gray-400 space-y-2 font-mono">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-brand-primary">{user?.roles?.[0] || 'member'}</span>
              </div>
            </div>
          </div>
