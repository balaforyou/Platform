...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      // F-220: the Guest Management "Setup Rules" sections call slot-engine. admin-web's
      // vite.config.ts has this entry; admin-v2 didn't until this screen needed it.
      // Production is unaffected - Caddy already routes /api/slot-engine/* globally.
      '/api/slot-engine': {
