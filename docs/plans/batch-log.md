...
## Batch — guest-member-pwa UI polish (Home / Slot & Time Selection / Review & Pay) + F-307

**Visual/layout pass across `apps/guest-member-pwa`, one branch/PR (`ui-polish-guest-pwa-26sep`)** per Bala's direct handover, with two deliberate non-visual exceptions called out rather than buried: the Home welcome-heading copy change and F-307 (checkbox → implicit consent). Plan-mode investigation caught two real handover errors before implementation, corrected in the open: Part 1's original target (`BranchDashboard.tsx`) is the per-venue court-pool listing page, not Home — the real Home route (`/`) is `main.tsx`'s `MainDashboard()`; and F-307's cited decision doc does not exist anywhere in the repo (Chief pasted the real content directly into the session instead, per the source-of-truth rule against inventing content for a missing reference).

**Global tokens** (`index.css`, `index.html`): `--color-bg` beige → `#F8FAFC`; `--font-heading` Caprasimo → Inter; `index.html`'s static `meta-theme-color` fallback → matches the new background (confirmed nothing updates it at runtime); JBC's real `Tenant.themeColor` row updated `#166534` → `#16A34A` (DB + `scripts/tenants/jbc.json` seed config, so a future rebuild doesn't silently restore the old value) — Home's primary CTA (`book-court-dashboard-btn`) explicitly repointed from the fixed gold ramp (`--color-accent-2-400`) to the tenant-derived `--color-accent-700`, confirmed it was never wired to the tenant ramp to begin with.

**Home** (`main.tsx`): welcome heading → Option B ("Welcome, {firstName}", Bala-confirmed), old separate "Hi, {firstName}" line removed as redundant; nameless-fallback ("Welcome!") added for the two Playwright fixtures that bypass real (Gmail-only) login with no displayName/email — real evidence during verification showed one of those two fixtures (`pwa-install-dismissal.spec.ts`, an existing seeded user with a real email) actually resolves via `/auth/refresh`'s email claim rather than hitting the fallback at all, corrected in the spec, not the component. Empty-state card restyled (`--mint-surface`, `rounded-2xl`); Shell's avatar touch target bumped 40px → 44px; bottom nav given a blurred/elevated background.

**Slot & Time Selection** (`BranchBooking.tsx`): dark venue-chip pill + separate about-badge row replaced with one sticky top bar (back button, title opens the venue switcher, subtitle+info icon opens the about sheet) — both real triggers relocated, not dropped; multi-pool switcher (`pools.length > 1` gate) confirmed untouched. Section labels de-jargonned ("01 · DAY" → "Select Date", "02 · START" → "Select Time"). Empty summary placeholder hidden on mobile only (dead space before a slot is picked) — kept on desktop; the existing real fixed bottom bar (time/duration/total/CTA) already IS the de facto bottom sheet on mobile once a slot is selected, so it was left intact rather than rebuilt into a stripped two-row sheet, which would have meant tearing out working duration-stepper/rate-source/booking-rules logic for a styling batch. No hold-timer badge added — confirmed no client-side hold-countdown state exists today (only a server-side 5-min TTL with no UI wire-up); flagged as new feature work, out of scope here.

**Review & Pay** (`BookingPay.tsx`): "Players" row replaced with a genuinely data-driven "Entire Court (Up to N players)" line using `booking.window.resourcePool.capacity`, already returned by `GET /bookings/:id` — no backend change needed. "Verified" phone badge now actually reads `user.isPhoneVerified` instead of rendering whenever `user.phone` is truthy. Pricing kept as a single "Amount to Pay" line — confirmed the `Booking` model has no separate fee/tax fields, so no tax line was fabricated. Court-name-in-header ask (`"JBC Main Courts • Court 1"`) deliberately not added — that row was intentionally omitted from this exact card by a prior finding (F-189) to avoid reading as double-charging next to the per-hour price; reversing it wasn't specifically re-confirmed, so left as-is.

**F-307** (checkbox → implicit consent, register row above): visible checkbox removed; passive disclaimer added carrying the full original clause set verbatim in substance (shoes; food **and** drinks; the liability waiver; real venue name), corrected during Chief's plan-approval review after a first draft silently narrowed it. New `ensureTermsAndIntent()` preserves the exact same two-call server order (`POST /bookings/:id/terms` then `POST /payment/intents`) the removed checkbox's `onChange` used to trigger, now fired from the Pay button and the dev-only simulate-payment button alike. `TERMS_VERSION` bumped.

**Evidence:** `npx tsc --noEmit` and `vite build` clean; built CSS grepped to confirm the new tokens landed and the old ones are gone (not just source-level). Full backend regression suite 5/5 sections (one run's `identity-auth`/`tenant-management` failures confirmed environmental on a clean re-run, matching this repo's documented identity-auth flakiness precedent). Real dev-stack e2e run (`pnpm dev:up`, Caddy-fronted, both tenants) against `badminton_db_e2e`: `pwa-install-dismissal.spec.ts` passing for real (including the corrected heading assertion); `findings-verification.spec.ts` and `f043-phase-c.spec.ts` and `f023-full-system.spec.ts` all pre-existing `describe.skip`/`test.skip`, confirmed unrelated to this batch; `guest-booking.spec.ts`'s one remaining failure (a slot-card timeout at a step this batch never touched) confirmed **pre-existing** via a real stash-and-rerun against unmodified `main` — identical failure with none of this batch's changes applied. Five real Playwright specs' selectors updated for the renamed top-bar classes and the removed checkbox (`#accept-terms-checkbox` click steps removed since the Pay/simulate button itself now triggers consent). JBC's real DB row confirmed holding `#16a34a` after an unrelated dev-stack container restart during verification (named-volume data intact).

**Real environment fact surfaced, not this batch's to fix:** `badminton_postgres` was found running on host port 65500 before this session (matching `.env`'s `DATABASE_URL`) rather than the canonical `65432` `docker-compose.yml` maps it to. Bringing up the full dev stack via `pnpm dev:up` for e2e recreated the container on the documented canonical port (data intact, same named volume) — `.env`'s port now disagrees with the running container until whichever session owns that drift reconciles it.

**Close-out:** `pnpm register:check` — **271 rows, Open 116 / Resolved 155** (was 270, Open 116 / Resolved 154 after the F-269 batch above: one new Resolved row, F-307; F-304/F-305/F-306 remain pending-findings-only, no register rows yet). Register row and pending-findings `Confirmed-ID: F-307` entry land in the same PR as the code, per the established convention. **Not yet merged — pending Bala/Chief sign-off per rule 6.**

## Batch — guest-member-pwa post-deploy feedback round 2 (Home/Slot & Time Selection/Review & Pay/History polish)

**Real live-testing feedback from Bala against the production deploy of the prior UI-polish batch, on branch `guest-pwa-feedback-round2-26sep`.** One real regression from that batch fixed (Shell.tsx's account-trigger button was bumped 40px→44px for touch-target size, but `<Avatar size={28}>` was never adjusted — bumped to 32 to restore the pre-batch proportion). Everything else is content/wording/layout/theming polish clarified directly with Bala via multiple AskUserQuestion rounds this session.

**Content/wording:** Home CTA `"+ New Booking"` → `"Book a Court +"` for guests (members keep `"+ Book as Guest"`); Home section header `"My Bookings"` → `"Current Bookings"`; footer nav tab and `BookingHistory.tsx`'s own page header both `"My Bookings"` → `"History"`; reserve button `"Hold & Proceed to Pay"` → `"Continue to Payment"`. Home's empty state rewritten to Bala's exact copy with a composed Feather+Calendar icon (no shuttlecock asset exists anywhere in the repo or in lucide-react). Home's populated bookings card now shows the real assigned court number (`booking.resource.name`) alongside the pool name — already fetched, no backend change.

**Layout:** cancellation policy moved from Slot & Time Selection to Review & Pay — required one additive backend include (`GET /bookings/:id`'s `resourcePool: true` → `resourcePool: { include: { bookingRules: true } }`). Redundant "TOTAL ₹N" block removed from the Pay sticky footer (the button already shows the amount). Slot & Time Selection's new top bar (from the prior batch) had two real bugs from Bala's screenshot: Shell.tsx's fixed avatar overlapping the top bar's text (fixed with reserved padding) and the pool-info card duplicating the venue name already shown in the top bar (fixed with a display-only dedup). **The dedup fix required a real correction caught in Chief review before implementation**: JBC's two real branches use different dash characters in the same seed-data position (Branch 1 has no dash; Branch 2's real name uses an en-dash, U+2013, where its pool name uses a plain hyphen, U+002D) — an exact string-prefix match would have silently failed on exactly the branch it exists to fix. Verified at the Unicode code-point level against `scripts/tenants/jbc.json`, not eyeballed. Fixed with dash normalization before comparing; live-verified against both real branches post-fix.

**Payment:** UPI added as the first, prioritized block in the Razorpay checkout config (`config.display`), compatible with the existing F-190/F-192 one-button decision. History's list sort changed to descending by real slot date (was backend-ordered by when the booking was *made*); Home's upcoming list confirmed to stay soonest-first (a real, deliberate distinction confirmed with Bala, not assumed).

**Theming — the largest item:** a full, explicit reversal of F-235 Phase 0's "fixed gold as the universal secondary accent" design, confirmed by Bala after seeing the real audit scope (not a misunderstanding — every `--color-accent-2-*` usage across ~13 real files replaced with the same-numbered-step tenant-green token: `main.tsx`, `BookingHistory.tsx`, `BookingConfirmation.tsx`'s entire hero, `CancelBookingModal.tsx`, `LoginScreen.tsx`, `AboutSheet.tsx`/`BranchAbout.tsx`/`BranchSelect.tsx`'s Clock icons, `BranchBooking.tsx`/`.css`, `BookingPay.tsx`, and `index.css`'s `--slot-available-accent` alias). Also fixed for completeness in two confirmed-dead-code files (`ui/Button.css`, `ui/StatusPill.css` — zero real importers anywhere in the app, zero production risk). Gold ramp's token definitions left in place, intentionally inert, not deleted.

**Evidence:** `npx tsc --noEmit` / `vite build` clean; built CSS and JS bundles grepped for `color-accent-2-` — confirmed zero remaining `var()` consumption anywhere in the compiled output (only the 3 expected definition blocks remain, matching source). Full backend regression suite 5/5 (the one backend change, the additive `bookingRules` include, is covered). Real dev-stack live-fire verification (JBC tenant, light mode, both real branches explicitly): Home, Slot & Time Selection (both branches — the en-dash dedup fix confirmed working on the exact branch it targets), Review & Pay (cancellation policy present, no TOTAL block, disclaimer green, real booking taken through Pay), Booking Confirmation (hero green), History (header/badges/button green, F-307's `termsAcceptedAt`/`termsVersion` DB read-back reconfirmed intact through this batch's changes).

**Close-out:** no register/finding ID needed — pure content/copy/theming/layout polish, same convention as the prior UI-polish batch (F-252/254/255/256/257 precedent). **Not yet committed or pushed — awaiting Bala/Chief sign-off per rule 6**, and no deploy without a separate explicit instruction, consistent with the prior round's practice.
