...
import {
  disconnectAvailabilityGenerationPrisma,
  ensureAvailabilityWindowsForDate,
} from './availabilityGeneration.js';
...
async function cleanup() {
  ...
  await db.bookingPlayer.deleteMany({
    where: { booking: { resourcePoolId: { in: poolIds } } },
  });
...
async function run() {
  ...
  assert(patternWindows.every((window) => window.generatedFromPatternId === pattern.id), 'pattern windows must carry provenance');
  assert(patternWindows.every((window) => Number(window.price) === 150), 'pattern windows must carry pattern price');
  ...
  assert(modifiedWindows[0].startTime.toISOString().includes('14:00:00.000Z'), 'modified windows must start at override start');
  assert(modifiedWindows.every((window) => window.capacity === 5), 'modified windows must use override capacity');
  assert(modifiedWindows.every((window) => window.generatedFromPatternId === null), 'modified windows must not claim pattern provenance');
  ...
  assert(raceResults.reduce((sum, result) => sum + result.createdCount, 0) === 3, 'only one concurrent call may create windows');
  ...
  assert(stableWindowAfter.startTime.getTime() === stableWindowBefore.startTime.getTime(), 'pattern edit must not change existing window start');
  assert(stableWindowAfter.endTime.getTime() === stableWindowBefore.endTime.getTime(), 'pattern edit must not change existing window end');
  assert(stableWindowAfter.capacity === stableWindowBefore.capacity, 'pattern edit must not change existing window capacity');
  assert(Number(stableWindowAfter.price) === Number(stableWindowBefore.price), 'pattern edit must not change existing window price');
  ...
  assert(Number(bookingAfter.price) === Number(bookingBefore.price), 'pattern edit must not change booking price');
...
run()
  .then(async () => {
    console.log('PHASE_A_AVAILABILITY_GENERATION_OK');
  })
  .catch((error) => {
    console.error('PHASE_A_AVAILABILITY_GENERATION_FAILED', error);
    process.exitCode = 1;
  })
  .finally(async () => {
