...
**Status:** Implemented and locally verified to the extent this environment allows. Committed to `f235-phase0` (`243b576`, `5584808`) and pushed to origin per your requests. **Not merged to `main`.** Correction 5 (§8) fixes the two dark-mode contrast issues reported in §3.4 — one was real, one was a mistake in my own original verification script. **§9 (new): Docker came up in this environment on a later check — real login through both apps' real channels, and the 4 previously-unrunnable Playwright specs, are now done with real evidence**, including one real (infra, not code) bug found and fixed along the way.
...
## 9. Docker check (follow-up) — real login, real Playwright runs, one real bug found and fixed

You asked me to check whether Docker Desktop was running now, and if so use it to close the evidence gaps from §3.2/§3.3/§3.5's "not reachable" items. It was up this time. Ran `dev-up.ps1` (the project's own documented way to bring up the full stack): 8 containers, `badminton_db` migrated — **this is also the first real, live confirmation that the hand-authored `20260916120000_booking_terms_acceptance_f235` migration (§1.5) actually applies cleanly**, against both `badminton_db` (via `dev-up.ps1`) and `badminton_db_e2e` (applied by hand before running Playwright, so the new columns exist there too).

### 9.1 Real login — both apps, both real channels (handover §6 item 2)

**guest-member-pwa, phone-OTP (`http://localhost:8080/?tenant=jbc`):** real `POST /api/identity/auth/otp/request` → 200, dev OTP `123456` read from `docker logs platform-identity-auth-1`, real `POST /api/identity/auth/otp/verify` → 201, landed on the real `MainDashboard` fully authenticated. This exercises Correction 1's guest-pwa-local `requestOtp`/`verifyOtp` end-to-end for real — confirms the fix that was previously only typecheck-verified actually works against the real backend.

**guest-member-pwa, `Shell`/`AccountSheet`/dark mode/logout, all live:** opened the real Account sheet (Correction 4), switched theme to Dark, and read the *actual rendered* `getComputedStyle` of the real "Home" nav link and its nav bar — `color: rgb(98, 171, 118)` (`#62ab76`) on `background-color: rgb(32, 29, 23)` (`#201d17`) — **the exact Correction-5-fixed pairing, confirmed on live rendered DOM, not just a CSS variable read.** Also confirmed `--color-accent-emphasis-dark` was set by `TenantContext` (via the real `emphasisBackgrounds` prop) to `#62ab76` — matching my earlier offline computation exactly. Signed out via the sheet's Sign Out button — real logout, redirected back to the login screen.

**admin-web, phone-OTP:** first attempt hit a real bug — see §9.2. After fixing it: real `POST /api/identity/auth/otp/request` → 200, dev OTP `123456`, real `POST /api/identity/auth/otp/verify` → 201, landed on the real admin dashboard (Overview/Resources/Scheduling/... nav, branch selector). Signed out and logged back in a second time to confirm it wasn't a fluke — same result both times.

**admin-web, mock-Google:** real `POST /api/identity/auth/google/verify` → 401, `{"code":"INVALID_GOOGLE_TOKEN","message":"Google ID token verification failed: Invalid Compact JWS"}`. **Confirmed pre-existing, not a regression from this branch** — `services/identity-auth/src/index.ts:701`'s own comment: *"Real Google ID token verification. No mock fallback in any environment (F-228 Decision 3)."* This is literally the F-228 incident `claude/guestPWA2/03-technical-lead-investigation-f235-auth-surface-fork.md` describes as the reason this whole auth-split exists — `admin-web`'s mock-Google button has called a real-verification-only endpoint since F-228 shipped, independent of anything in this branch. My refactor moved the exact same (already-broken) call verbatim; re-tested after clearing the stale-cache bug in §9.2 to make sure the fresh code path gives the identical result, which it does.

**guest-member-pwa, real Google Sign-In:** still not verified — this needs an actual Google account completing a real OAuth consent screen, which isn't something to automate in this session (no real account available, and driving a real user through Google's own login UI programmatically is out of bounds here regardless). Needs a real person with a real Google account, same as the charter's real-device requirement.

### 9.2 One real bug found and fixed — stale Vite dependency cache, not a code defect

First `admin-web` phone-OTP attempt failed in the browser with **`setSession is not a function`** even though the backend call succeeded (`otp/verify` → 201). Root cause, confirmed by direct inspection: `docker exec platform-admin-web-1`'s `node_modules/.vite/deps/@badminton_ui-shared.js` (Vite's dependency-optimizer cache) still contained the *old* `requestOtp`/`verifyGoogleMock` shape — it pre-dated the `AuthContext` trim and had never been invalidated, even though the actual mounted source file (`/app/packages/ui-shared/dist/context/AuthContext.js`, confirmed via `docker exec ... cat`) was current. This is a Vite dependency pre-bundling cache staying stale across a long-running dev container, not a defect in the auth-split code itself — `guest-member-pwa`'s container didn't hit this (its own `.vite/deps` cache had no stale entry), so it wasn't a symptom of any error in my code.

**Fix:** `docker exec platform-admin-web-1 rm -rf node_modules/.vite && docker restart platform-admin-web-1`. Re-tested immediately after — real login through both admin-web channels now behaves as reported above (OTP succeeds twice in a row; mock-Google fails with the identical pre-existing 401, ruling out the stale cache as the cause of *that* failure too). Flagging this because it's a real thing that happened, not because it needs a code change — nothing in this repo caused it; it's a known category of Vite dev-server behavior when a workspace dependency's build output changes underneath a long-running container.

### 9.3 The 4 previously-unrunnable Playwright specs — real results

Ran against the real stack (fresh containers, after §9.2's fix), `badminton_db_e2e` migrated to include the new terms columns:

| Spec | Result | Why |
|---|---|---|
| `findings-verification.spec.ts` | Skipped (not failed) | Pre-existing, unrelated to this branch: the test itself is `test.skip(...)` — F-009's client-side coverage was deliberately dropped when F-114 removed co-player collection. Confirmed by reading the spec file directly; nothing to do with F-235. |
| `guest-booking.spec.ts` | **Failed — exactly the expected reason.** | `expect(page).toHaveURL('/branches')` — real error: `Expected: "http://localhost:8080/branches", Received: "http://localhost:8080/"`. This is Correction 3, confirmed live: `/branches` no longer exists, the catch-all route sends it to `/`. |
| `f043-phase-c.spec.ts` | Failed — **environmental, not evaluated for real** | `Error: listen EADDRINUSE: address already in use :::8080`. This spec spawns its *own* local service processes and its own proxy on port 8080/3001-3003 rather than using the already-running docker stack — colliding with `dev-up.ps1`'s own Caddy/services on those same ports. This is a real collision, but caused by running both stacks at once, not by anything in this branch; I did not tear down the docker stack to get a clean read on this one, given time — flagging rather than claiming a result I don't have. |
| `f023-full-system.spec.ts` | Failed — **reproduced twice, same way; root cause not fully pinned down, but not an app bug** | Both runs: `Test timeout of 180000ms exceeded` on `noSessionPage.waitForResponse(...)` waiting for `/api/slot-engine/member/today-assignment` (line 264), immediately after a fresh `loginByOtp` for a member with no session today. **But the actual page state at the moment of timeout (captured both times, identical) shows the login and the API call already fully succeeded** — a correctly-rendered `MainDashboard` reading "Signed in as: +919833333333", "Account type: member", "No pre-scheduled matches today" (i.e. the *response* to that exact API call already rendered correctly). Playwright's `waitForResponse` promise, attached immediately before the login navigation, simply never resolved despite the real request/response cycle demonstrably completing. I could not pin down why Playwright's own listener missed a network event that the app's own rendered state confirms happened — reporting this honestly as unresolved rather than guessing. **Separately, and regardless of the above:** this same spec references `/branches/${branchId}/book/${poolBId}` later in its file (line 411-412), a section it never reached this run — so even if the line-264 issue is fixed, this spec would still fail later for the same Correction-3 reason as `guest-booking.spec.ts`. |

Net: 1 confirmed-pre-existing skip, 1 confirmed-expected route-collapse failure, 1 environmental collision I didn't resolve, 1 reproducible-but-unexplained Playwright-level timeout that the app's own real rendered state suggests is not an application bug. None of the four point to a new regression in the actual application code from this branch.
