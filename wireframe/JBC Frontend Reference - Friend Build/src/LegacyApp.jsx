...
export default function App() {
  ...
  const [authState, setAuthState] = useState('loggedOut'); // loggedOut | biometricsPrompt | loggedIn
...
=========================================`;
  ...
  return (
    ...
    <div style={badmintonBgStyle} className={`min-h-screen flex flex-col font-sans relative transition-all duration-350 ${
      ...
      <footer className={`fixed bottom-0 left-0 right-0 max-w-md mx-auto border-t grid py-2 px-4 z-40 shadow-xl transition-colors ${styles.footer} ${
        role === 'member' ? 'grid-cols-3' : 'grid-cols-4'
      }`}>
        {role === 'member' ? (
          <>
            <button 
          ...
          <>
            ...
            <button 
              ...
              onClick={() => navigateTo('adminApps')}
              className={`flex flex-col items-center gap-0.5 py-1.5 transition cursor-pointer ${
                currentScreen === 'adminApps' || currentScreen === 'adminGuestBookings' || currentScreen === 'adminStudentTraining' || currentScreen === 'adminTournaments' || currentScreen === 'adminAddMember' || currentScreen === 'adminGroupManager' || currentScreen === 'adminAnalytics'
                  ? 'text-emerald-500 font-black scale-105' 
                  : `${styles.textMuted} hover:text-slate-350`
              }`}
            >
